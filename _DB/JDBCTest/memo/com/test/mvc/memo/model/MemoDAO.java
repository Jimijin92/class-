...
public class MemoDAO {
	...
	public MemoDAO() {
		try {
			...
			System.out.println("MemoDAO.Contructor : " + e.toString());
		}
	}

	//MemoClass가 메모 입력내용을 주고 DB에 Insert해달라고..  
	public int add(MemoDTO dto) {
		...
		try {
			...
			PreparedStatement stat
			  	= conn.prepareStatement(ds.get("memo.updatepoint"));
			
			stat.setInt(1, 100);//메모 쓰기 +100포인트
	...
	public String getMseq(String seq) {
		...
		try {
			...
			PreparedStatement stat
				= conn.prepareStatement(ds.get("memo.getMseq"));
	...
	public void del(String seq) {
		...
		try {
			...
			//메모 삭제
			PreparedStatement stat
				= conn.prepareStatement(ds.get("memo.del"));
			stat.setString(1, seq);
			stat.executeUpdate();
