...
- 그룹을 왜 묶을까?
   ...
   ☆★☆★ 3. 각각의 그룹별로 집계함수를 적용하기 위해서 
   
- group by 키 
    : 그룹을 나누는데 기준이 되는 컬럼(1개 or 그 이상)
...
SELECT buseo, avg(basicpay) 
    FROM tblinsa
        WHERE basicpay >=2000000 -->요 구문있는지 없는지에 따라 결과값 달라짐, 아직 group by오기전이니까 모든 직원한테 물어보는 셈     
            GROUP BY buseo;
    

-- 조건 x : 지역별로 그룹 나눈 뒤 각 지역당 직원들 중 가장 월급이 많은 사람의 급여는 ?
-- 조건 o : 대리와 사원들에 한해서 지역별로 그룹 나눈 뒤 
SELECT city, max(basicpay), count(*)
    FROM tblinsa
        WHERE jikwi in('대리', '사원') --대리랑 사원 없는 지역은 안나타남 
            GROUP BY city;
...
SELECT buseo, round(avg(basicpay)) FROM tblinsa
    GROUP BY buseo
        HAVING avg(basicpay)>=1500000; --집계함수의 결과나 그룹바이의 대상으로만 조건으로 걸수 있음 주로 집계함수로 조건검
