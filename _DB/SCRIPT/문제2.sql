...
SELECT*FROM tabs;
-- 03. tblinsa 테이블 구조 확인 (SQL plus에서만 가능)
DESC tblinsa;
-- 04. tblinsa 테이블의 이름(name), 기본급(basicpay) 조회
SELECT name as 이름, basicpay as 기본급 FROM tblinsa;
-- 05. tblinsa 테이블의 이름(name), 기본급(basicpay), 수당(sudang), 기본급+수당 조회
SELECT name as 이름, basicpay as 기본급, sudang as 수당, basicpay+sudang as "기본급+수당" FROM tblinsa; 
...
SELECT*FROM tblinsa WHERE city = '인천' and basicpay BETWEEN '1000000' AND '2000000';
...
SELECT name as 이름, basicpay as 기본급, sudang as 수당, basicpay+sudang as "기본급+수당" FROM tblinsa WHERE basicpay+sudang >='2500000';
-- 14. 주민번호를 기준으로 남자(성별란이 1, 3)만 조회. (이름(name), 주민번호(ssn)). 
--     단, SUBSTR() 함수 이용.
SELECT name as 이름, ssn as 주민번호 FROM tblinsa WHERE SUBSTR(SSN,8,1)IN('3','1');
-- 15. 주민번호를 기준으로 80년대 태어난 사람만 출력. (이름(name), 주민번호(ssn)). 
--     단, SUBSTR() 함수 이용.
SELECT name as 이름, ssn as 주민번호 FROM tblinsa WHERE SUBSTR(SSN,1,1)='8';
-- 16. 서울 사람 중에서 70년대 태어난 사람만 출력. SUBSTR() 함수 이용.
SELECT name as 이름, ssn as 주민번호 FROM tblinsa WHERE SUBSTR(SSN,1,1)='7' AND city='서울';
-- 17. 서울 사람 중에서 70년대 태어난 남자만 출력. SUBSTR() 함수 이용.
SELECT name as 이름, ssn as 주민번호 FROM tblinsa WHERE SUBSTR(SSN,1,1)='7' AND city='서울' AND SUBSTR(SSN,8,1)IN('3','1');
-- 18. 서울 사람이면서 김씨만 출력(성씨가 1자라는 가정). (이름, 출신도). 
-- SUBSTR() 함수 이용.
SELECT name as 이름, city as 출신도 FROM tblinsa WHERE SUBSTR(name,1,1)='김' AND city='서울';
...
SELECT name as 이름, city as 출신도, ibsadate as 입사일 FROM tblinsa WHERE SUBSTR(ibsadate,1,1) = '2' AND SUBSTR(ibsadate,6,2) = '10'; 
...
SELECT name as 이름, ssn as 주민번호 FROM tblinsa WHERE SUBSTR(ssn,4,1) = '5';
-- 24. 주민번호 기준으로 5월달생만 출력. TO_CHAR() 함수 이용.
-- 문자열 -> 날짜 -> 문자열
SELECT name as 이름, ssn as 주민번호 FROM tblinsa WHERE TO_CHAR(SSN,'999999-9999999');
-- 25. 출신도 내림차순으로 정렬하고, 출신도가 같으면 기본급 내림차순 --> 각각 오름차순인지, 내림차순인지 적어주기
SELECT num, name, ssn, ibsadate, city as "city(내림차순)", tel, buseo, jikwi, basicpay as "basicpay(내림차순)", sudang FROM tblinsa 
        ORDER BY city DESC, basicpay DESC;
-- 26. 서울 사람 중에서 기본급+수당(->급여) 내림차순으로 정렬. 
-- (이름, 출신도, 기본급+수당)
SELECT name as 이름, city as 출신도, basicpay+sudang as "기본급+수당" FROM tblinsa ORDER BY basicpay+sudang DESC;
-- 27. 여자 중 부서 오름차순으로 정렬하고, 부서가 같으면 기본급 내림차순 정렬. (이름, 주민번호, 부서, 기본급)
SELECT name as 이름, ssn as 주민번호, buseo as 부서, basicpay as 기본급 
        FROM tblinsa WHERE SUBSTR(SSN,8,1)IN('2','4') 
                ORDER BY buseo ASC, basicpay DESC;
-- 28. 남자 중에서 나이를 기준으로 오름차순 정렬해서 출력.
SELECT*FROM tblinsa ORDER BY ssn DESC;
-- 29. 서울 지역 사람들 중에서 입사일이 빠른 사람을 먼저 출력.
SELECT*FROM tblinsa WHERE city = '서울' ORDER BY IBSADATE ASC;
-- 30. 성씨가 김씨가 아닌 사람 출력. (이름, 출신도, 기본급). 
-- LIKE 연산자 또는 SUBSTR() 함수 이용.
SELECT name, city, basicpay FROM tblinsa WHERE NOT SUBSTR(name,1,1) ='김';
-- 31. 출신도가 서울, 부산, 대구 이면서 
-- 전화번호에 5 또는 7이 포함된 자료 출력하되 
-- 부서명의 마지막 부는 출력되지 않도록함. 
-- (이름, 출신도, 부서명, 전화번호)
SELECT name, city, SUBSTR(buseo,1,2) as 부서, tel FROM tblinsa 
...
--GROUP BY 구문
SELECT count(*), floor((basicpay+sudang)/1000000)*100 || '만원' FROM tblinsa
    GROUP BY floor((basicpay+sudang)/1000000);

-- 34. tblinsa 테이블에서 주민번호를 가지고 생년월일의 년도별 직원수 출력
SELECT count(*), SUBSTR(SSN,1,2) FROM tblinsa
...
FROM tblinsa;
-- 39. 서울 사람의 남자 인원수 출력.
SELECT count(*) FROM tblinsa WHERE city = '서울' AND SUBSTR(SSN,8,1)IN('3','1');
...
SELECT
      avg(decode(substr(ssn, 8, 1), '1', basicpay)) as 남자평균값,
      avg(decode(substr(ssn, 8, 1), '2', basicpay)) as 여자평균값
FROM tblinsa;
...
SELECT 
        ...
        count(decode(SUBSTR(SSN,8,1),'2',1)) as 여자인원수,
        count(decode(SUBSTR(SSN,8,1),'1',1)) as 남자인원수,
        sum(decode(SUBSTR(SSN,8,1),'2',basicpay)) as 여자급여합,
        sum(decode(SUBSTR(SSN,8,1),'1',basicpay)) as 남자급여합
...
-- 51. 부서별 남,여 인원수 출력. 인원수가 5명 이상인 부서만 출력.
SELECT buseo,
        count(decode(SUBSTR(SSN,8,1),'2',1)) as 여자인원수,
        count(decode(SUBSTR(SSN,8,1),'1',1)) as 남자인원수
FROM tblinsa
        ...
        AND count(decode(SUBSTR(SSN,8,1),'1',1))>=5;

-- 52. 이름, 성별, 나이 출력
--        성별: 주민번호 1,3->남자, 2,4->여자 (DECODE 사용)
--        나이: 주민번호 이용해서
SELECT name, 
        decode(SUBSTR(ssn, 8,1),'1','남자', '3', '남자', '2', '여자', '4', '여자')as 성별
...
--         1월    10명
--         2월    25명
SELECT TO_CHAR(ibsadate, 'mm')||'월' as 월, count(*)||'명' FROM tblinsa
...
SELECT name, '19' || SUBSTR(SSN, 1, 2) || '-' || SUBSTR(SSN, 3, 2) || '-' || SUBSTR(SSN, 5, 2) as 생년월일, TO_CHAR(basicpay,'L9,999,999') as 기본급, sudang as 수당 FROM tblinsa;
...
SELECT name, ibsadate FROM tblinsa
WHERE to_char(sysdate,'yyyy') - SUBSTR(ibsadate,1,4)> 15;
...
SELECT name, ssn, city FROM tblinsa WHERE to_number(substr(ssn, 1, 2)) BETWEEN 75 AND 82;

-- 60. 근무년수가 15~20년인 사람 출력. (이름, 입사일)
SELECT name, ibsadate FROM tblinsa WHERE (to_char(sysdate,'yyyy')- to_number(substr(ibsadate, 1, 4))) BETWEEN 15 AND 20;
...
SELECT name, buseo FROM tblinsa WHERE name LIKE '김%'  OR name LIKE '박%' OR name LIKE '이%';
...
SELECT name, ssn, to_char(ibsadate,'yyyy')||'년 '||to_char(ibsadate, 'mon')||' '||to_char(ibsadate,'dd')||'일 '||to_char(ibsadate,'day') FROM tblinsa
...
SELECT buseo, jikwi, count(*) as 인원수, sum(basicpay) as 급여합, round(avg(basicpay)) as 급여평균 FROM tblinsa
GROUP BY buseo, jikwi
ORDER BY buseo ASC, jikwi ASC;
...
--         전체 2000 1999 1998
--          60    x    x    x
SELECT count(*), count(decode(to_char(ibsadate,'yyyy'),'2000',1)) as "2000",
                                count(decode(to_char(ibsadate,'yyyy'),'1999',1)) as "1999",
                                count(decode(to_char(ibsadate,'yyyy'),'1998',1))  as "1998" FROM tblinsa;
...
-- 84. 부서별 기본급이 가장 높은 사람 출력. (이름, 부서, 기본급)        
      ...
      FROM(SELECT buseo, max(basicpay) as m FROM tblinsa GROUP BY buseo) a;
...
CASE
        ...
        WHEN avg(basicpay) BETWEEN 1500000 AND 2000000 THEN 'B'       
        ELSE 'C'
END  as 등급
FROM tblinsa 
        GROUP BY buseo;
...
--방법 2
SELECT name, pay  FROM 
        (SELECT name, (basicpay + sudang) as pay FROM tblinsa
                ORDER BY pay DESC)
                        WHERE rownum = 1;
                        
-- 96. tblinsa. 80년대생 남자 직원들의 평균 월급(basicpay)보다 더 많이 받는 70년대생 여직원들을 출력.
SELECT * FROM tblinsa 
        WHERE SUBSTR(SSN, 8, 1) = '2' AND SUBSTR(SSN,1,1) = '7' 
                AND basicpay > (SELECT avg(basicpay) FROM tblinsa WHERE SUBSTR(SSN, 8, 1) = '1' AND SUBSTR(SSN,1,1) = '8');
...
SELECT address FROM tblmember WHERE byear = (SELECT min(byear)FROM tblinsa); 
