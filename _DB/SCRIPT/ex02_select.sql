...
2. 객체 식별자 : 소문자(풀)
    ...
    b. 나머지 타입
        - INTEGER, DOUBLE, DECIMAK, REAL 등... : 다른 시스템 or 예전 버전들과의 호환성 때문에.. 
    2. 문자형
    - 자바 : 문자(char) + 문자열(String)
    - 오라클은 문자와 문자열의 구분이 없다. 모두다 문자형으로 취급한다. 
    ...
    a. CHAR/NCHAR
        ...
        ex) CHAR(3) : 3바이트까지 문자를 저장(abc ->, 홍길동-> ) UTF8이면 OK UTF16이면 6바이트라서 에러! --문자길이가 아니라 바이트 수임
        --한번잡으면 남아있는공간은 회수가 안되고 그대로 유지 
        -공간낭비 but 일 절약
        - 최대크기 :2000바이트
        - 최소크기 :1바이트
    ...
    ex)CHAR(3) : UTF-8 3바이트 -> 이게 더 많이 쓰임 
    2. N이 있으면 : 오라클의 기본인코딩과 상관없이 무조건 UTF-16을 사용 
    ex)NCHAR(3) : 3글자까지 //물리적으로는 6바이트 
...
INSERT INTO tbltype (name) VALUES ('일이삼사오육칠팔구십'); -- O
INSERT INTO tbltype (name) VALUES ('일이삼사오육칠팔구십일'); -- O

INSERT INTO tbltype (name) VALUES ('일이삼사오육칠팔구 ab'); -- O
INSERT INTO tbltype (name) VALUES ('일이삼사오육칠팔구 abc'); -- O
-- 결론 : 숫자고 문자고 길이만 잘 맞추면 돼 

INSERT INTO tbltype (name) VALUES ('홍길동','홍길동'); 
