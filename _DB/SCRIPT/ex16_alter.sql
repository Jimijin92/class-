...
ALTER TABLE tbledit
        ADD(color VARCHAR2(30) DEFAULT '미정' NOT NULL); -- 모든 기존 레코드의 새컬럼에 default 가 적용된다. 
...
ALTER TABLE tbledit 
        MODIFY(price NUMBER(4) NOT NULL); --주의: 기존값들이 전부 있다고 가정

--ADD (color VARCHAR2(30) DEFAULT '미정' NOT NULL);
ALTER TABLE tbldit
        MODIFY(color VARCHAR2(30) DEFAULT '미정'
                        CHECK(color IN ('미정','검정','노랑','빨강','파랑')));

INSERT INTO tbledit VALUES (4, '메모리', 2000, '녹색');
...
3. TRUNCATE 명령어 (수학 함수 TRUNC 함수와 다른 명령어)
- 정식 (***)
- 테이블의 모든 행을 삭제하는 명령어 (DELETE 조건절없이 사용하는 것과 동일 = DELETE*)
- 되돌리기 불가능 (복구 불가능) 
*/
