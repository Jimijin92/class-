...
--컬럼명은 상관없음 
SELECT name, jikwi FROM 과장 
UNION
SELECT name as nick, jikwi as position FROM 부장;

-- 컬럼갯수가 다름 -> 컬럼 갯수는 동일해야 한다. 
SELECT name, jikwi, city FROM 과장 
UNION
SELECT name, jikwi FROM 부장;
...
UNION --수학의 합집합개념(중복값은 제거 - distinct)
