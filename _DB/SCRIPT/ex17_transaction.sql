...
-- 현재 트랜잭션의 모든 내용을 DB에 적용시키고 트랜잭션 완료 + 새 트랜잭션 시작 
COMMIT; 
...
2. COMMIT 실행 직 후
3. ROLLBACK 실행 직 후 
4. DDL, DCL실행 직 후 (자동 COMMIT)

자동 커밋, Auto  Commit
- 클라이언트 툴의 기능이 아님;;
- 사용자가(현재 접속중이 세션에서 명시적으로 COMMIT을 실행하지 않아도 자동으로 COMMIT이 실행되는 상황
- DDL, DCL을 실행할 때 사용자 모르게 자동으로 COMMIT이 발생한다. 
