...
- 도식 > ERD(Entity Relationship Diagram)
        ...
        a. 데이터베이스 모델링
                - 가장 초반 작업
                - 데이터베이스 구성 요소 > 테이블 도식화 > 모델링 최종 산출물 > ERD
...
ERD에서 Entity, Attribute, Relationship을 표현하는 방법(그리는 방법)
1. Entity
- 사각형 표시
- 이름은 대문자
- 중복 불가능
...
키, KEY
- 역할(의미) 부여된 속성(Attribute), 컬럼(Column)
...
4. 외래키, Foreign Key
- 부모 테이블의 기본키를 참조하는 키
- 테이블(Relation)(엔티티 표현)간의 관계(Relationship)(테이블과 테이블사이 선)을 표현하는 역할
...
이상 현상(Anomaly)
- 발생 이유 -> 테이블 설계가 잘못되어서... -> 이상 현상을 발생하지 않도록 테이블 바로잡자 -> 정규화 
...
- 동일한 데이터가 2개 이상의 테이블에 동시에 존재 -> 1개의 테이블 데이터 수정 
        -> 반드시 수정 + 마침 -> 다른 테이블의 데이터와 상태가 달라지는 경우

함수 종속(Functional Dependency)
- 하나의 테이블 내에서 2개 이상의 속성간의 관계 제약
...
- 1개의 셀안에 2개 이상의 복합 데이터가 보이면 
        -> 새롭게 컬럼을 나눠서 단일값(1개의 컬럼 = 1개의 값)으로 바꾼다. 
        *** 가로로 컬럼을 늘리면 -> NULL 발생
...
- 부분 함수 종속을 발견해서 제거한다.(***)
- 복합키를 가지는 테이블에서만 발생한다. (PK가 1개인 테이블에서는 발생 안함)

제 3정규화, 3NF
- 기본키가 아닌 다른컬럼에 의존하면 안된다.
