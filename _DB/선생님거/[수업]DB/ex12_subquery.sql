...
SELECT
      CASE
            WHEN (max(adddate) > max(completedate)) THEN max(adddate)
            ELSE max(completedate)
      END
FROM tbltodo;
...
SELECT * FROM tblinsa
      WHERE basicpay >  (SELECT basicpay FROM tblinsa WHERE jikwi = '과장');
...
-- 기획부 직원들은 길동이보더 얼마를 더 받고 얼마를 덜 받는지 차액이 궁금?
...
SELECT * FROM tblinsa
      WHERE (buseo, jikwi) = (SELECT jikwi, buseo FROM tblinsa WHERE name = '나윤균');
