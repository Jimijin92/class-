...
- 하나의 테이블에 데이터 많이 저장 -> 초반에 있는 데이터는 접근 속도 빠름, 후반에 있는 데이터는 접근 속도 느림 -> 자주 검색되는 대상 컬럼 -> 인덱스 생성 -> 데이터 접근 : 표 사용X, 인덱스 사용O -> 원하는 레코드의 접근 속도 향상
- 오라클은 테이블 생성 시 인덱스를 명시적으로 생성하지 않아도 자동으로 생성된다. -> PK, Unique 제약이 붙은 컬럼은 자동으로 인덱스가 만들어진다. -> PK, Unique 컬럼들은 where 절에서 사용될 확률이 높아서..
*/
...
CREATE INDEX idx_tblinsa_salary
      ON tblinsa(basicpay + sudang > 1500000); --WHERE basicpay + sudang > 1500000
