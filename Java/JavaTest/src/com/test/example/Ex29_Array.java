...
public class Ex29_Array {
	...
	private static void m8() {
		...
		String[] adress = new String[count];
		...
		for (int i=0; i<100; i++) {
			...
			// 0 -> 남자, 1 -> 여자
			//System.out.println(rnd.nextInt(2) == 0 ? "남자" : "여자" );
			...
			//주소
			adress[i] = a1[rnd.nextInt(a1.length)]
	...
	private static void m7() {
		...
		ns5 = {95, 57, 3, 39, 22}; 이렇게 두 문 장으로는 인식 못함*/
		
		boolean[] flags = {true, false, false, true, false};
		
		String[] names = {"홍길동", "아무개", "호호호", "테스트"};
		/*String[] names = {
							"홍길동"
							, "아무개"
							, "호호호"
		...
		int [] seq1 = new int[3]; // 주로 사용
		int seq2 [] = new int[3]; // 요새 잘 안씀 /사용하지 말기
	...
	private static void m6() {
		...
		//단점
		//1. 요소의 접근을 순차적으로밖에 못한다. 0~마지막. (짝수만, 홀수만, 거꾸로 이런거X)
		//2. 읽기 전용(요소의 값을 수정하는 용도로는 사용 불가능)
		//nums 안에 들어있는 방 하나와 같은 크기의 변수 생성
		for(int n : nums) {//num찾아가서 0번째 방부터 찾아서 값 찾아서 n에 집어넣음(방향1, 하는일1, //그래서 빠름)
			System.out.println(n);
		}
		...
		for(int n : nums) {
			System.out.println(n);
			//n = n * 3; 읽기 전용이라 이런식으로 값 수정 못함 (일반 for문은 가능)
		}
	...
	private static void m4() {
		...
		//배열의 길이를 정적으로 할당했다.
		int[] kors = new int[300];//적절하게 예측해서 살짝 여유 있게 만들어두기
	...
	private static void m3() {
		...
		//실수 배열
		double[] list2 = new double[3];  //더블 배열
		list2[0] = 3.14; // 더블 타입
		//list2 = 3.14;
		System.out.println(list2[0]);
		...
		Scanner[] scans = new Scanner[3];//스캐너를 넣을 수 있는 방 세개 
	...
	private static void m2() {
		...
		//에러메세지는 더 구체적인게 위에 뜸
		//System.out.println(nums[5]);(범위 밖)
