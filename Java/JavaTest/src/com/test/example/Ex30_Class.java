...
public class Ex30_Class {
	...
	public static void main(String[] args) {
		 ...
		 *
		 * 2. 객체(개체), Object
		 * 	- 데이터(나를 구성하는 모든 정보) + 행동(나의 움직임) = 객체(행동은 데이터를 기반으로 한다.)
		 *  - 변수 + 메소드 = 객체
		 *  
		 *  - 독자성,Identity
		 *  - 상태, State 
		 ...
		 *  3. 클래스, Class
		 *  	- 객체를 생성하는 설계도
		 *  	- 개발자 -> 클래스 설계(선언) -> 나중에 객체가 생성될 때 객체가 소유하게 될 데이터와 행동을 정의하는 것
		 *  	  -> 클래스를 사용해서 객체 생성 -> 객체를 사용해서 목적에 맞는 업무를 구현 
		 *  
		 ...
		 *****************************자바에서 클래스 만들기************************************
		 ...
		 3. 자바 파일(*.java) 1개당 클래스 1개 선언
		 	- 필수(X) -> 권장(O)
		 	- 소스 파일 관리가 쉬워진다.
		 	- 1개의 .java파일내에 여러개의 클래스를 선언하는 경우
