...
import com.teacher.ComTeacherCalendar;

public class ComStudentCalendar{
	...
	public static void studentdiary() {
		...
		String sNumber = ComRegisterLogin.getUserEndemic();
		int roomNumber = 0;
		...
		//------------------------------------추가-----------------------------------------
		ArrayList<String> schNum = new ArrayList<String>();	//이거는 이 메소드 맨 밑에서 student 에게 토스!
		
		for(int i=0; i<Schedulelist.size(); i++) {
			if(sNumber.equals(Schedulelist.get(i).getSNumber())) {
				roomNumber = i;
				myclasslist.add(roomNumber+"");
				schNum.add(Schedulelist.get(i).getClassNumber());
			}
		}
...
//	System.out.println(TutoringDayAllStudent);
		...
		int lank = 1;
		System.out.println("   [수업시간]              [선생님이름]               [과목]               [가격]             [과외 시작일]");
...
//													
//							if((Schedulelist.get(i).getSNumber()).equals(sNumber)) {
...
//						System.out.println(j); //스케쥴이 들어있는 방번호 에서 j번방의 넘버 10, 10번쨰 방의 정보 불러오기		
						...
						for(int n=0; n<TeacherList.size(); n++) {
							if((TeacherList.get(n).getTNumber()).equals(Schedulelist.get(findNum).getTNumber())) {
